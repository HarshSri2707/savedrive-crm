...
const STATUS_STYLES = {
  ...
  CONTACTED: "bg-[#FFF7ED] text-[var(--orange-dark)]",
  QUALIFIED: "bg-[#F5F3FF] text-[#7C3AED]",
  CLOSED: "bg-[var(--teal-light)] text-[var(--teal-dark)]",
};
...
export default function LeadDetailModal({ leadId, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/leads/${leadId}`)
      ...
      .catch(() => {
        if (!cancelled) setError("Couldn't load lead details.");
      })
      .finally(() => {
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span
                className={`shrink-0 text-[0.7rem] font-bold px-3 py-1 rounded-full ${
                  STATUS_STYLES[lead.status] || "bg-[var(--gray-100)] text-[var(--gray-600)]"
                }`}
              >
                {lead.status}
              </span>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Created Date" value={formatDate(lead.createdAt)} />
            </div>
          </div>
        ) : null}
      </div>
